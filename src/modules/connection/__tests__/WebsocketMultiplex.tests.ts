...
describe("WebsocketMultiplex", function () {
  test("socketDemuxer:eats characters upto end if length is incorrect", function () {
    const data = new Uint8Array([
      40, 40, 0, 0, 5, 0, 0, 0, 99, 104, 97, 110, 101, 118, 101, 110, 116, 97,
      115, 100, 115,
    ]);
  ...
  test("sockerMuxer:writes data correctly for bytes", function () {
    const buf = socketMuxer(
      "chan",
      "event",
      new TextEncoder().encode("asds").buffer
    );
    const data = new Uint8Array([
      4, 0, 0, 0, 5, 0, 0, 0, 99, 104, 97, 110, 101, 118, 101, 110, 116, 97,
      115, 100, 115,
  ...
  test("expect mux and demux conversion", function () {
    expect(socketDemuxer(socketMuxer("chan", "event", "asds").buffer)).toEqual({
      channelId: "chan",
      eventName: "event",
